"use client";

import { Input } from "@/components/ui/input";
import { CirclePercent, Heart, ShoppingCart, UserRound } from "lucide-react";
import Link from "next/link";
import { useCart, useHeart } from "./zust";

export default function Navbar() {
  const { heart } = useHeart();
  const { cart } = useCart();

  return (
    <div className="flex flex-col gap-4 fixed w-full bg-white/85 backdrop-blur-xl pb-3 z-50">
      <section className="h-8 flex items-center justify-center bg-purple-800">
        <p className="font-dmSans text-white text-xs font-medium">
          Free delivery & 40% discount for next 3 orders! Place your 1st order
          in
        </p>
      </section>
      <section className="px-20 flex gap-4 items-center justify-between">
        <img src="/logo.svg" alt="Freshly logo" className="h-6" />
        <Input
          type="search"
          placeholder="Type your products"
          className="h-8 text-sm focus:shadow-none max-w-90 bg-white"
        />
        <div className="flex gap-4 items-center">
          <div className="rounded-full bg-gray-100 p-2">
            <UserRound className="opacity-75" size={20} />
          </div>
          <div className="flex items-start gap-1 min-w-8">
            <p className="font-medium opacity-80 font-dmSans top-2 right-2 text-sm">
              {heart.length > 0 && heart.length}
            </p>
            <Heart
              className="opacity-75"
              size={18}
              color={heart.length > 0 ? "transparent" : "black"}
              fill={heart.length > 0 ? "red" : "transparent"}
            />
          </div>
          <div className="flex items-start gap-1 min-w-8">
            <p className="font-medium opacity-80 font-dmSans top-2 right-2 text-sm">
              {cart.length > 0 && cart.length}
            </p>
            <ShoppingCart
              className="opacity-75"
              size={18}
              color={cart.length > 0 ? "green" : "black"}
              fill={cart.length > 0 ? "green" : "transparent"}
            />
          </div>
        </div>
      </section>
      <section className="font-dmSans text-xs flex justify-between mx-20">
        <div className="flex gap-6 font-medium border border-neutral-300 rounded py-3 px-4 w-fit hover:bg-white duration-200">
          <Link
            href={"/"}
            className="hover:text-green-900 hover:scale-110 duration-200"
          >
            Home
          </Link>
          <Link
            href={"/"}
            className="hover:text-green-900 hover:scale-110 duration-200"
          >
            Pages
          </Link>
          <Link
            href={"/"}
            className="hover:text-green-900 hover:scale-110 duration-200"
          >
            Shop
          </Link>
          <Link
            href={"/"}
            className="hover:text-green-900 hover:scale-110 duration-200"
          >
            Vendor
          </Link>
          <Link
            href={"/"}
            className="hover:text-green-900 hover:scale-110 duration-200"
          >
            Elements
          </Link>
          <Link
            href={"/"}
            className="hover:text-green-900 hover:scale-110 duration-200"
          >
            Blog
          </Link>
          <Link
            href={"/"}
            className="hover:text-green-900 hover:scale-110 duration-200"
          >
            Contact
          </Link>
        </div>
        <div className="text-xs flex font-medium">
          <div className="flex gap-1 justify-center items-center w-34">
            <CirclePercent size={15} />
            <p>Weekly discount</p>
          </div>
          <div className=" text-white bg-purple-800 flex flex-col w-30 h-full justify-center items-center rounded-r-md">
            {/* <p className="">Hotline Number</p> */}
            <p>+9888-256-666</p>
          </div>
        </div>
      </section>
    </div>
  );
}
